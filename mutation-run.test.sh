...
out=$("$SCRIPT" --repo "$FIX" 2>&1); rc=$?
if (( rc == 0 )) && [ "$(cat "$FIX/got.txt" 2>/dev/null)" = "test.sh" ]; then ok "{files} scopes the test command"; else fail_msg "files scope — rc=$rc out=$out got=$(cat "$FIX/got.txt" 2>/dev/null)"; fi
...
PATCH
orig_victim=$(cat "$OUTSIDE/victim.js")
out=$("$SCRIPT" --repo "$FIX" --diff "$TMPALL/attack.patch" 2>&1); rc=$?
if (( rc == 0 )) && [[ "$out" == *"resolves outside repo"* ]] && [ "$(cat "$OUTSIDE/victim.js")" = "$orig_victim" ]; then
  ok "a path resolving outside the repo is rejected before it is read"
else
  fail_msg "symlinked path was not rejected on containment — rc=$rc out=$out victim=$(cat "$OUTSIDE/victim.js" 2>/dev/null)"
fi
...
other=$(git -C "$SCRIPT_DIR" status --porcelain | grep -vxF '?? mutation-run' | grep -vxF '?? mutation-run.test.sh' || true)
