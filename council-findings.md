...
scripts/agent-devbox/bootstrap.sh:91 — `cat > ~/.agentrc` truncates the file on every run, so a re-run (advertised as idempotent) after the user follows README Phase 4 and puts `SUPERSET_API_KEY` in `~/.agentrc` wipes that secret and any other local edits -> only create/write `~/.agentrc` when missing, or manage keys in a separate file the script never overwrites.
