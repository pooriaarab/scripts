...
CFG = {"prefix": "scr"}
json.dump({"encoding": "base64", "content": base64.b64encode(json.dumps(CFG).encode()).decode()}, open(os.path.join(T, "pr-standards-config.json"), "w"))
...
def write_config(overrides): json.dump({"encoding": "base64", "content": base64.b64encode(json.dumps(overrides).encode()).decode()}, open(os.path.join(T, "pr-standards-config.json"), "w"))
...
reports = {"valid": doc(), "stale": doc(tested_head="a"*40), "prhead": doc(), "fg": doc(), "pend": doc(), "omit": doc(),
           ...
           "draftpr": doc(), "closedpr": doc(), "badconfig": doc(), "configcap": doc()}
...
cases = [
    ...
    ("draft live PR is rejected", "draftpr", (), 1, "draft and not reviewable"),
...
def sync_report(name, case_head):
    pl = json.load(open(os.path.join(T, "reports", name + ".json"))); pl["head"] = pl["tested_head"] = pl["ci"]["head"] = case_head
...
def run_case(name, extra=(), after=None):
    ...
    write_config(CFG)
    if name == "badconfig": open(os.path.join(T, "pr-standards-config.json"), "w").write('{"encoding":"base64","content":"' + base64.b64encode(b"{").decode() + '"}')
    if name == "configcap": write_config({"prefix": "scr", "maxLines": 0, "maxFiles": 40})
    if name == "bun":
